...
struct SimplifyRedundantTranspose : public mlir::OpRewritePattern<TransposeOp> {
  ...
  matchAndRewrite(TransposeOp op,
     ...
     *
     */
    TransposeOp transpose_input_op = llvm::dyn_cast_or_null<TransposeOp>(transpose_input.getDefiningOp());//dynamic_cast用于类继承层次间的指针或引用转换。主要还是用于执行“安全的向下转型（safe downcasting）”，也即是基类对象的指针或引用转换为同一继承层次的其他指针或引用。
    if(!transpose_input_op)
    {
    	return failure();
    }
