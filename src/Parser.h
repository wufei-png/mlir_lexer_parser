...
private:
  ...
  std::unique_ptr<VarType> parseType() {
    ...
    */
    ...
    if (lexer.getCurToken() == '[')
    {
    	...
    	bool ismedium = true;	//判断位于哪个[]了
	while (lexer.getCurToken() == tok_number)//这里的getCurToken是从Lexer中得到的
	{
		type->shape.push_back(lexer.getValue());
		lexer.getNextToken();
		if (lexer.getCurToken() == ']')//第一个或者第二个[]结束了
		{
			if(ismedium)
			{//说明第一个[]结束了，是第一个[]，如果是false，直接到break了，因为是第二个了
				lexer.getNextToken();
				ismedium = false;
				if (lexer.getCurToken() == '[')
				{
					lexer.getNextToken();//到第二个[]了
				}
			}
			else
			{
  ...
  parseVarDeclaration(bool requiresInitializer) {
    ...
    if (lexer.getCurToken() != tok_var)
    {
    	return parseError<VarDeclExprAST>("var", "to begin var declaration");
    }
    auto loc = lexer.getLastLocation();
